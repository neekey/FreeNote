...
(function($){
  ...
  function longTap(){
    if (touch.last && (Date.now() - touch.last >= longTapDelay)) {
      $(touch.target).trigger('longTap', touch );
      touch = {};
    }
  }
  ...
  $(document).ready(function(){
    $(document.body).bind('touchstart', function(e){
      ...
      if (touch.isDoubleTap) {
        $(touch.target).trigger('doubleTap', touch );
        touch = {};
      } else if (touch.x2 > 0 || touch.y2 > 0) {
        (Math.abs(touch.x1 - touch.x2) > 30 || Math.abs(touch.y1 - touch.y2) > 30)  &&
          $(touch.target).trigger('swipe', touch ) &&
          $(touch.target).trigger('swipe' + (swipeDirection(touch.x1, touch.x2, touch.y1, touch.y2)), touch );
        touch.x1 = touch.x2 = touch.y1 = touch.y2 = touch.last = 0;
      } else if ('last' in touch) {
        touchTimeout = setTimeout(function(){
          touchTimeout = null;
          $(touch.target).trigger('tap', touch )
          touch = {};
        }, 250);
      }
